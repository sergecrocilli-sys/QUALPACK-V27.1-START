...
(function (global, factory) {
  ...
  function buildXlsx(workbook) {
    ...
    for (let si = 0; si < workbook.SheetNames.length; si++) {
      ...
      for (let r = 0; r < aoa.length; r++) {
        ...
        for (let c = 0; c < row.length; c++) {
          ...
          // QualPack : support minimal des styles cellule via objet { v, s }
          // s = index de style Excel (0 défaut, 1 date, 2 TU1, 3 TU2)
          let styleId = null;
          if (val && typeof val === 'object' && !(val instanceof Date) && Object.prototype.hasOwnProperty.call(val, 'v')) {
            styleId = Number.isFinite(Number(val.s)) ? Number(val.s) : null;
            val = val.v;
          }
          ...
          if (typeof val === 'number') {
            wsXml += `<c r="${ref}"${styleAttr}><v>${val}</v></c>`;
          } else if (typeof val === 'boolean') {
            wsXml += `<c r="${ref}"${styleAttr} t="b"><v>${val ? 1 : 0}</v></c>`;
          } else if (val instanceof Date) {
            const serial = dateToSerial(val);
            wsXml += `<c r="${ref}" s="${styleId !== null ? styleId : 1}"><v>${serial}</v></c>`; // s="1" → style date
          } else {
            // String → shared string
            const idx = getSSIdx(String(val));
            wsXml += `<c r="${ref}"${styleAttr} t="s"><v>${idx}</v></c>`;
          }
        }
        wsXml += `</row>`;
      }
    ...
    // xl/styles.xml — style minimal + format date + couleurs QualPack TU1/TU2
    // Styles cellule : 0 défaut, 1 date, 2 TU1 jaune, 3 TU2 rouge
